...
from compiler.front_end.abstract_nodes import *
...
class SymbolCollector(Decorator):
    ...
    ########################################################################################################################
    def function_definition_pre(self, node:NormalDeclaration, children: list[ASTNode]):
        #
        self.stack.enter_scope(ScopeKind.FUNCTION)


    def function_definition_post(self, node: NormalDeclaration, children: list[ASTNode]):
        self.stack.exit_scope()
    ########################################################################################################################
